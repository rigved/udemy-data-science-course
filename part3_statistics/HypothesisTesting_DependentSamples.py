"""
HypothesisTesting_DependentSamples.py
...
data = pd.read_excel('data/HypothesisTesting_DependentSamples.xlsx', sheet_name='Magnesium', skiprows=list(range(3)), usecols='B:C')
...
print('At 5% significance in a one-tailed test, we ' + decision + ' the null hypothesis that the magnesium levels have reduced.')

decision = 'accept'
if abs(T_score) > t_score_2:
    decision = 'reject'

print('At 1% significance in a one-tailed test, we ' + decision + ' the null hypothesis that the magnesium levels have reduced.')
