"""
ConfidenceIntervalsExercise.py
...
df = pd.read_excel('data/ConfidenceIntervalsExercise.xlsx', sheet_name='Al Bundy', skiprows=[0, 1, 2], parse_dates=[1], index_col=0, dayfirst=True, usecols='A:O')
...
months = sorted(set(df.loc[df['Year'].isin([2015, 2016])]['Month']))
confidence_level = 0.95

data_male_united_states1_monthly = pd.DataFrame()
data_female_united_states1_monthly = pd.DataFrame()
data_male_united_states2_monthly = pd.DataFrame()
data_female_united_states2_monthly = pd.DataFrame()

for month in months:
    data_male_united_states1_monthly = pd.concat([data_male_united_states1_monthly, df.loc[(df['Year'] == 2015) & (df['Month'] == month) & (df['Gender'] == 'Male') & (df['Country'] == 'United States')]['Size (US)'].value_counts(sort=False, dropna=False)], axis=1, ignore_index=True)
    data_male_united_states2_monthly = pd.concat([data_male_united_states2_monthly, df.loc[(df['Year'] == 2016) & (df['Month'] == month) & (df['Gender'] == 'Male') & (df['Country'] == 'United States')]['Size (US)'].value_counts(sort=False, dropna=False)], axis=1, ignore_index=True)
    data_female_united_states1_monthly = pd.concat([data_female_united_states1_monthly, df.loc[(df['Year'] == 2015) & (df['Month'] == month) & (df['Gender'] == 'Female') & (df['Country'] == 'United States')]['Size (US)'].value_counts(sort=False, dropna=False)], axis=1, ignore_index=True)
    data_female_united_states2_monthly = pd.concat([data_female_united_states2_monthly, df.loc[(df['Year'] == 2016) & (df['Month'] == month) & (df['Gender'] == 'Female') & (df['Country'] == 'United States')]['Size (US)'].value_counts(sort=False, dropna=False)], axis=1, ignore_index=True)

months_2015 = [np.int64('2015' + str(i)) for i in months]
months_2016 = [np.int64('2015' + str(i)) for i in months]

data_male_united_states1_monthly.columns = months_2015
data_male_united_states2_monthly.columns = months_2016
data_female_united_states1_monthly.columns = months_2015
data_female_united_states2_monthly.columns = months_2016

data_male_united_states1_monthly.fillna(value=0, inplace=True)
data_male_united_states2_monthly.fillna(value=0, inplace=True)
data_female_united_states1_monthly.fillna(value=0, inplace=True)
data_female_united_states2_monthly.fillna(value=0, inplace=True)

data_male_united_states1_monthly.index.name = 'Male Size (US) 2015'
data_male_united_states1_monthly.sort_index(inplace=True)
data_male_united_states2_monthly.index.name = 'Male Size (US) 2016'
data_male_united_states2_monthly.sort_index(inplace=True)

data_female_united_states1_monthly.index.name = 'Female Size (US) 2015'
data_female_united_states1_monthly.sort_index(inplace=True)
data_female_united_states2_monthly.index.name = 'Female Size (US) 2016'
data_female_united_states2_monthly.sort_index(inplace=True)

print(data_male_united_states1_monthly)
print(data_male_united_states2_monthly)
print(data_female_united_states1_monthly)
print(data_female_united_states2_monthly)

data_male_united_states_monthly = pd.concat([data_male_united_states1_monthly, data_male_united_states2_monthly], axis=1, ignore_index=True)
data_female_united_states_monthly = pd.concat([data_female_united_states1_monthly, data_female_united_states2_monthly], axis=1, ignore_index=True)

data_male_united_states_monthly.columns = data_male_united_states1_monthly.columns.append(data_male_united_states2_monthly.columns)
data_female_united_states_monthly.columns = data_female_united_states1_monthly.columns.append(data_female_united_states2_monthly.columns)

data_male_united_states_monthly.index.name = 'Male Size (US) 2015-2016'
data_female_united_states_monthly.index.name = 'Female Size (US) 2015-2016'
...
data_male_united_states_monthly['Standard Error'] = [round(np.std(data_male_united_states_monthly.loc[i, list(months_2015 + months_2016)], ddof=1) / np.sqrt(data_male_united_states_monthly.shape[1] - 1), 2) for i in data_male_united_states_monthly.index]
data_female_united_states_monthly['Standard Error'] = [round(np.std(data_female_united_states_monthly.loc[i, list(months_2015 + months_2016)], ddof=1) / np.sqrt(data_female_united_states_monthly.shape[1] - 1), 2) for i in data_female_united_states_monthly.index]
