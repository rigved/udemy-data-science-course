"""
HypothesisTesting_PopulationVarianceUnknown.py
...
data = pd.read_excel('data/HypothesisTesting_PopulationVarianceUnknown.xlsx', sheet_name='Open rate dataset', skiprows=[0, 1,  2], usecols='B')
...
t_score_3 = t.ppf(confidence_level_1, df=(data.shape[0] - 1))
t_score_4 = t.ppf(confidence_level_2, df=(data.shape[0] - 1))
...
print('One-sided test t-score for {}% confidence level with {} degrees of freedom: {:.2f}'.format(int(confidence_level_1 * 100), data.shape[0] - 1, t_score_3))
print('One-sided test t-score for {}% confidence level with {} degrees of freedom: {:.2f}'.format(int(confidence_level_2 * 100), data.shape[0] - 1, t_score_4))
...
print('At 5% significance in a one-sided test, we ' + decision + ' the null hypothesis that the average open rate for our emails is {}%.'.format(mean_h0 * 100))

decision = 'accept'
if abs(T_score) > t_score_4:
    decision = 'reject'

print('At 1% significance in a one-sided test, we ' + decision + ' the null hypothesis that the average open rate for our emails is {}%'.format(mean_h0 * 100))

print('T-statistic for two-sided test: {:.2f}'.format(t_statistic[0]))
print('p-value for two-sided test: {:.3f}'.format(p_value[0]))
print('p-value for one-sided test: {:.3f}'.format(p_value[0] / 2))
